...
Aan het onderzoek namen acht vrouwen deel. Vier vrouwen waren langer dan 12 maanden gediagnosticeerd met reumatoïde artritis (RA) en waren ACPA-positief. De overige vier vrouwen vormden de ACPA-negatieve controlegroep. Van alle deelnemers werd een synoviale biopsie afgenomen. De RNA-sequencingdata waren afkomstig van Platzer et al. (2019) en werden geanalyseerd in R.
Het humane referentiegenoom GRCh38.p14 (GCF_000001405.40) werd geïndexeerd en de paired-end reads werden uitgelijnd met Rsubread (v2.24.0). Met featureCounts werd vervolgens een gen-level countmatrix opgesteld. Deze werd geanalyseerd met DESeq2 (v1.50.2) om differentieel geëxprimeerde genen tussen de RA- en controlegroep te identificeren (padj < 0,05 en |log2FC| > 1). De resultaten werden gevisualiseerd met EnhancedVolcano (v1.28.2).
Voor functionele verrijkingsanalyse werden de significante genen omgezet naar Entrez-ID's met org.Hs.eg.db en geanalyseerd met clusterProfiler (v4.18.4). Hierbij werden GO- en KEGG-verrijkingsanalyses uitgevoerd. Het specifieke KEGG-pathway Rheumatoid arthritis (hsa05323) werd vervolgens met pathview (v1.50.0) gevisualiseerd, waarbij de log2 fold change van de genen binnen het pathway werd weergegeven.
